...
export const RiderApp: React.FC<RiderAppProps> = ({
  ...
  const pendingRequests = rider.kycStatus === 'approved' ? orders.filter((o) => o.status === 'pending') : [];
          ...
          <nav className="flex space-x-2 sm:space-x-8 overflow-x-auto py-2.5">
            ...
            >
              ...
              {pendingRequests.length > 0 && (
                <span className="bg-white/20 text-white px-2 py-0.2 rounded-full font-extrabold text-[11px]">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        ...
        {rider.kycStatus === 'approved' && (rider.walletBalance || 0) > 0 && (rider.walletBalance || 0) <= 20 && (
          <div className="bg-orange-50 border border-orange-300 text-orange-900 p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs">
            ...
            {rider.kycStatus !== 'approved' ? (
              <div className="bg-white p-12 rounded-2xl border border-amber-300 text-center space-y-3 shadow-sm">
                <ShieldCheck className="w-12 h-12 text-amber-500 mx-auto" />
                <h3 className="text-base font-bold text-slate-900 font-heading">Pending Order Requests Hidden</h3>
                <p className="text-xs text-slate-600 max-w-md mx-auto">
                  New delivery requests are hidden for newly joined partners until your KYC documents are reviewed and approved by the admin in the admin panel.
                </p>
                <button
                  type="button"
                  onClick={() => setActiveTab('profile')}
                  className="mt-2 px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-xl transition-colors cursor-pointer inline-flex items-center gap-1.5 shadow-2xs"
                >
                  <User className="w-3.5 h-3.5" />
                  <span>Review Partner Profile & Documents</span>
                </button>
              </div>
            ) : !isOnline ? (
              <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center space-y-3">
                <p className="text-sm font-bold text-slate-700">You are currently OFFLINE</p>
                <p className="text-xs text-slate-500">Switch status to ONLINE to accept incoming delivery requests.</p>
                <button
                  onClick={() => handleToggleOnline(true)}
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs cursor-pointer transition-colors shadow-xs"
                >
                  Go Online Now
                </button>
              </div>
              ...
              <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center space-y-3">
                ...
                <p className="text-xs text-slate-500">
                  New orders created by customers will appear in this vertical feed automatically.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4 animate-in fade-in zoom-in-95">
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-xl text-xs transition-all shadow-md flex items-center justify-center gap-1.5 cursor-pointer"
                    >
